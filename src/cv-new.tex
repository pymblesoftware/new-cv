...
\textbf{ PostgreSQL/TimescaleDb (Contract) } \\
\textbf {NBN} %  \textbf { Sadaqa Welfare Fund}. \\ %  ( www.sadaqa.org.au )\\
\\
PostgreSQL, Timescale DB, Patroni, HA Proxy, PG Bouncer, etcd, Docker, AWS EC2 \\
\\
Set up Timescale DB. \\
Database schema ingestion. Flyway configuration. \\
Set up replication to other Docker containers on other EC2 instances. \\
Designed architecture for HA Proxy, PG bouncer, Patroni, and etcd. \\
Configured and tested HA Proxy, PG bouncer, Patroni, and etcd.  \\

Recommendations for point-in-time recovery by replicating the write-ahead log to pgbacrest. \\
Recommendations from timescaledb-tune, leveraging continuous aggregates, targeted indexing and psql explain analyse for tuning. \\

Benchmarked different configurations with pgbench. \\
