...
Comments:	{\bf Maintenance} work covering for a staff member on leave. Data migration wrapped a SOAP layer around an XML-RPC-like application for interfacing with the \textbf{SAP PI SOAP interface}. Miscellaneous Apache/Linux fixing. \\
Created SAP PI/PO integration scenario with SOAP adapter configuration for inbound XML message processing. Developed interface mappings and routing rules to transform and route delivery notification and invoice data from third-party system to SAP ERP, supporting automated goods receipt posting and financial document creation for cement logistics operations \\
\\
...
Technologies:	C++, Solaris, Windows. Sybase, Sunsoft C++, Cytrix, Borland C++ Builder, Java, PERL/Tk, Tools++, DBTools++, GreenLeaf Comm++, SNA, LU-62, Systematics
